...
def clusterScoring(cLabels, tLabels, docDist, cDensity, cTexts, seedTweetVecs, feaVecs):
    clusterScore = []
    clusterScoreDetail = []
...
def clusterTweets(algor, documents, feaVecs, clusterArg):
    ...
    if algor == "kmeans" or algor == "default":
        ...
        clusterModel = cluster.DBSCAN(eps=clusterArg, min_samples=5, metric='euclidean', algorithm='auto', n_jobs=5).fit(feaVecs)
    ...
    if -1 in cLabels: cLabels.remove(-1)
    ...
    return cLabels, tLabels, docDist, cDensity, cTexts, cDocs_zip


def clustering(algor, documents, feaVecs, clusterArg, topK_c, topK_t, burstySeqIdArr, seedTweetVecs):

    if algor == "agg" and issparse(feaVecs):
        feaVecs = feaVecs.toarray()
    cLabels, tLabels, docDist, cDensity, cTexts, cDocs_zip = clusterTweets(algor, documents, feaVecs, clusterArg)
    print "## Clustering done. algorithm", algor, " #cluster", len(cLabels), time.asctime()
    if len(cLabels) < 5:
        return None, None
    # scoring clusters
    clusterScore, clusterScoreDetail = clusterScoring(cLabels, tLabels, docDist, cDensity, cTexts, seedTweetVecs, feaVecs)
